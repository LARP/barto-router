...
def run_stress_test():
    print("=" * 60)
    print("   BATERÍA DE PRUEBAS DE CARGA — BARTO-ROUTER")
    print("=" * 60)
    
    # Prueba 1: Prompt Corto
    ...
    test_single_prompt(p2, "PRUEBA 2: Carga Pesada (>5 KB de código)")

    # Prueba 3: Concurrencia
    print("\n--- PRUEBA 3: Concurrencia (3 peticiones en paralelo) ---")
    threads = []
    
    def worker(idx):
        p = f"Dame un consejo numero {idx} para optimizar shaders y draw calls en Unity URP para moviles."
        test_single_prompt(p, f"Hilo #{idx}")
    ...
    print(f"\nConcurrencia completada en {total_time:.2f}s para 3 solicitudes.")
    print("=" * 60)
